...
def get_mask_on_img(img, mask):
    ...
    rgb_img = hu_to_grayscale(img,-512,512)/255.

    mask_slice = np.where(mask == 1, 1, 0)
#     mask_slice_kid = np.where(mask == 2, 1, 0) # could be developed in order to plot both tumor and kidney masks on one image.

    rgb_mask = np.zeros((mask.shape[0], mask.shape[1], 3))
    rgb_mask[:, :, 0] = mask_slice
